...
static void test_valid(void)
{
    ...
    check(write_file("CFG1.TMP",
        ...
        "packetint 0x60\r\n"
        "IPADDR = 192.168.7.42 # workstation\r\n") == 0,
        "write valid fixture");
    result = dm_mtcp_config_load("CFG1.TMP", ip, &packet_int, &found);
    check(result == DM_MTCP_OK, "valid configuration accepted");
    check(found == (DM_MTCP_HAVE_IP | DM_MTCP_HAVE_PACKET_INT),
        "both known keys found");
...
static void test_last_value_wins(void)
{
    ...
    result = dm_mtcp_config_load("CFG2.TMP", ip, &packet_int, &found);
...
static void test_missing_key(void)
{
    ...
    result = dm_mtcp_config_load("CFG3.TMP", ip, &packet_int, &found);
...
static void test_invalid_values_are_atomic(void)
{
    ...
    result = dm_mtcp_config_load("CFG4.TMP", ip, &packet_int, &found);
    ...
    result = dm_mtcp_config_load("CFG5.TMP", ip, &packet_int, &found);
...
static void test_long_line(void)
{
    ...
    fputc('\n', stream);
    fclose(stream);
    result = dm_mtcp_config_load("CFG6.TMP", ip, &packet_int, &found);
